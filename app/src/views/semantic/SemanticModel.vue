    ...
    <section class="table-section">
      ...
      <div v-if="filteredVocabularies.length > 0" class="pagination">
        <span class="pagination-info">
          {{ t('common.showing') }} {{ paginationStart }}-{{ paginationEnd }} /
          {{ t('common.total') }} {{ filteredVocabularies.length }} {{ t('common.items') }}
        </span>
...
<script setup lang="ts">
  ...
  const paginationEnd = computed(() =>
    Math.min(currentPage.value * pageSize.value, filteredVocabularies.value.length)
  )
