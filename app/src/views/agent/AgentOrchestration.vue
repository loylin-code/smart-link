...
<script setup lang="ts">
  ...
  function handleBack() {
    if (isCreateMode.value) {
      ...
      if (confirm(t('common.confirm') + ' - ' + t('agent.wizard.nav.cancel') + '?')) {
        router.push('/app/agent/design/wizard')
      }
    } else {
    ...
    try {
      if (isCreateMode.value && schema) {
        ...
        const newAgent = await agentStore.createAgent({
          name: wizardData.value.name || t('agent.untitled'),
          code: `agent_${Date.now()}`,
          description: '',
          avatar: wizardData.value.avatar,
          tags: wizardData.value.tags
        })

        if (newAgent) {
          // Save schema to session storage for wizard to pick up
          sessionStorage.setItem('temp_schema', JSON.stringify(schema))
          sessionStorage.setItem('temp_agent_id', newAgent.id)
          // Navigate back to wizard publish step
          router.push('/app/agent/design/wizard')
        }
      } else if (isEditMode.value && agentId.value && schema) {
        // 更新现有智能体
        await agentStore.updateAgent(agentId.value, { pageSchema: schema })
      } else {
  ...
  onMounted(async () => {
    ...
    const editAgentId = route.params.id as string
    if (editAgentId && editAgentId !== 'create') {
      isEditMode.value = true
      agentId.value = editAgentId

      // Load agent data
      const agent = await agentStore.fetchAgent(editAgentId)
      if (agent?.pageSchema) {
        // Initialize editor with existing schema
        store.initSchema(agent.pageSchema)
      }
    } else {
      // 初始化空 Schema
      store.initSchema()
